...
On the use of Qiskit Metal coded in Python to generate design files for building quantum devices on a chip, performed via direct-write lithography. Depending on the resolution of features, LASER, scanning thermal probe, and electron-beam techniques are applicable options. (Note: in the patterned example below, the ground contacts were excluded from layout to save patterning time. However, the main features are clearly visible under optical microscopy. Also, the GDS design output for a full quantum chip design is available for download in the file directories above).
